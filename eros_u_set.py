...
if __name__ == '__main__':
    ...
    output_file_name = 'eros_gp_u_set_' + percentage + '.csv'
    path = LAB_PATH + 'Samples_Features/EROS/' + percentage + '%/'
    ...
    linea.append('class')
    linea = ','.join(linea) + '\n'
    f = open(TRAINING_SETS_DIR_PATH + 'EROS_GP/' + output_file_name, 'w')
    f.write(linea)
    f.close()

    lineas = [calc_gaussian(f) for f in files]

    f = open(TRAINING_SETS_DIR_PATH + 'EROS_GP/' + output_file_name, 'a')
    f.writelines(lineas)
    f.close()
