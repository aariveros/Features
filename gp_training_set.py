...
for i in range(len(paths)):
    try:
        ...
        for s in samples:
            fs = FeatureSpace(featureList=feature_names, Beyond1Std=s[1], MaxSlope=x, Eta_e=x)
        ...
        file_path = 'GP Samples/60/' + lc_class + '/' + macho_id + '.txt'
        ...
        print inst
        #Escribir lista con las curvas que fallaron
        f = open('GP Samples/60/error.txt', 'a')
        f.write(path + '\n')
        f.close()
        continue
